---
name: devkit-feature-lifecycle
description: >
  Generic feature lifecycle orchestration from branch start to ready-for-review state.
  Uses devkit-git-workflow as the baseline for git operations and quality gates.
triggers:
...
Note: Git commands, branch conventions and push safety rules are inherited from `../devkit-git-workflow/SKILL.md`.
...
- Ensure workflow references `devkit-git-workflow` as the source of git rules.
