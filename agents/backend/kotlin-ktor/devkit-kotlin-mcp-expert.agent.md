...
description: >
  ...
  Delegates quality audit to devkit-kotlin-server-quality after implementation.
...
- User asks for design pattern selection → delegate to `devkit-kotlin-expert-pattern`
- User asks for CI/CD pipeline → delegate to `devkit-devops`
- User asks for code quality audit → delegate to `devkit-kotlin-server-quality`
...
Always apply skill `devkit-postgresql-crud` for detailed patterns.
...
Always apply skill `devkit-unit-testing-kotlin` for detailed patterns.
...
> "Implementation complete. Recommend running `devkit-kotlin-server-quality` to validate code quality, detect layer violations, and confirm JaCoCo threshold."
...
When the user asks to **create**, **scaffold**, **generate**, or **bootstrap** a new Kotlin MCP server project, delegate to the [devkit-kotlin-mcp-server-generator](skills/backend/kotlin-ktor/devkit-kotlin-mcp-server-generator/SKILL.md) skill. This skill produces a complete production-ready project including Gradle files, server bootstrap, tools, tests, and README aligned with the official `io.modelcontextprotocol:kotlin-sdk`.

You can also invoke it directly via the slash command: `/kotlin-mcp-development:devkit-kotlin-mcp-server-generator`
